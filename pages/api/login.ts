...
const { JsonRpcProvider: JSONRPCProvider } = providers
...
const handler = async (
  ...
  try {
    ...
    const siwe: SIWEMessage = await message.validate(signature, mainnetProvider)
    ...
    const session: {
      ...
      nonce: null,
      cookie: {
        expires: new Date(Number(siwe.expirationTime)),
      },
    }
    const ens = await mainnetProvider.lookupAddress(siwe.address)
    if (ens) {
